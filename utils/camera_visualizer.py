...
def render_cameras(training_cameras = [], testing_cameras = [], render_image = False, hide_name = False):
    scale   = []
    poses   = []
    legends = []
    colors  = []
    images  = None if not render_image else []
    for training_camera in training_cameras:
        poses.append(training_camera.view_world_transform.T.squeeze().cpu().numpy())
        legends.append(training_camera.image_name if not hide_name else "")
        colors.append('blue')
        if render_image:
            images.append((training_camera.original_image.squeeze().permute(1, 2, 0) * 255.).to(torch.uint8).cpu().numpy())
        scale.append(max(np.abs(poses[-1][:3, 3]).tolist()))
    for testing_camera in testing_cameras:
        poses.append(testing_camera.view_world_transform.T.squeeze().cpu().numpy())
        legends.append(testing_camera.image_name if not hide_name else "")
        colors.append('green')
        if render_image:
            images.append((testing_camera.original_image.squeeze().permute(1, 2, 0) * 255.).to(torch.uint8).cpu().numpy())
        scale.append(max(np.abs(poses[-1][:3, 3]).tolist()))
    scale = np.ceil(max(scale)) + 1.
    return CameraVisualizer(poses, legends, colors, images).update_figure(scale, base_radius=1, zoom_scale=1, show_grid=True, show_ticklabels=True, show_background=True)